 ...
 *
 * Two halves: the mapping itself on today's fields, and the three Server
 * Actions run on ONE valuation with `buildDocumentModel` observed, proving
 * they hand it the same input (the preview's `approvedAt` — today — being the
 * only intended difference, spec §C).
 */
...
describe("approve / sign / preview hand buildDocumentModel the same input (R-2)", () => {
  ...
  it("builds one input for all three, apart from the preview's date", async () => {
    ...
    const sign = await inputOf(() => signValuationAction("v1"));

    const withoutDate = (input: typeof approve.input) => ({ ...input, approvedAt: null });
    expect(withoutDate(approve.input)).toStrictEqual(withoutDate(sign.input));
    expect(withoutDate(preview.input)).toStrictEqual(withoutDate(sign.input));

    // Sign re-renders on the persisted issue date; approve on the `now` it
    // persists; only the preview may carry a flag.
    expect(sign.input.approvedAt).toEqual(APPROVED_AT);
    ...
    expect(sign.opts).toBeUndefined();
    expect(preview.opts).toEqual({ preview: true });
  });

  it("each action assembles the input through documentInputFor, not by hand", () => {
    for (const file of ["approve-valuation.ts", "sign-valuation.ts", "preview-operat.ts"]) {
      const source = fs.readFileSync(path.join(__dirname, "../src/app/actions", file), "utf8");
      expect(source, file).toMatch(/\bdocumentInputFor\(/);
      expect(source, file).not.toMatch(/kwNumber:\s*valuation\.kwNumber/);
    }
  });
});
