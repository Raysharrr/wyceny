...
export async function savePool(
  ...
  await storage.put(poolKey(valuationId), gzipSync(Buffer.from(JSON.stringify(pool), "utf8")));
}

/**
 * Reloads the cached pool, or `null` when nothing was ever cached for this
 * valuation (a draft started before Slice 3, or a cleared storage) — the
 * ONLY case `reselectSample` treats as "pobierz ponownie", never a silent
 * re-selection on an empty pool (team-lead condition 1). A corrupt cache
 * (bad gzip, malformed JSON, or a shape `candidatePoolSchema` rejects) is a
 * genuine failure and is left to throw, not masked as a missing pool.
 */
export async function loadPool(
  ...
  try {
    ...
    return candidatePoolSchema.parse(JSON.parse(raw));
  } catch (e) {
