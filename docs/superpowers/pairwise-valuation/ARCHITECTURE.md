...
1. `valuation-input.ts` przejmuje współdzielone typy z kcs.ts; kcs.ts re-eksportuje historyczne nazwy, `KcsInput` pozostaje aliasem zgodności. `valuation-calculation.ts` zawiera jeden wybór algorytmu i politykę minimum/maksimum. Silniki czyste; KCS bez zmiany wzorów/zaokrągleń. S1 posiada progi i calculationIssues; S2 tylko podpina. Produkcyjne akcje nie wołają computeKcs bezpośrednio ani bramek na surowej puli; wspólny resolver wskazuje próbę używaną przez metodę. Polityka nowych operacji jest osobna od arytmetyki potrzebnej do odtworzenia archiwum.
2. Jawny wybór metody jest osobną mutacją, nie efektem zapisu próby. Brak→kcs na starym szkicu zachowuje WR i prozę, rzeczywista zmiana metody unieważnia. Rozszerzenie istniejącego JSON: metoda, potwierdzenie, dane ocen/poprawek PP; żadnej migracji wszystkich historycznych wierszy. Brak metody oznacza KCS przy odczycie/arytmetyce, ale nowa operacja zatwierdzenia szkicu wymaga jawnego wyboru. Odczyt zatwierdzonych/podpisanych danych ich nie uzupełnia ani nie zapisuje.
3. `comparables` zachowuje pełne dotychczasowe wiersze, a `pairwise.selectedComparableIds` wskazuje przyjęte3–5. Wspólny resolver `valuationComparables` karmi bramki, silnik, prozę i dokument; przełączenie do KCS nie usuwa ręcznych wierszy. Tożsamość transakcji: źródło + composite transactionId/lokalId lub coopTxId; przestrzenie id rozłączne rcn:/sm:/manual:. Schemat przepuszcza id, serwer weryfikuje unikalność i nadaje brakujące trwałe id także wierszom ze źródła z niepełnymi identyfikatorami. Zachowujemy ceny już zapisane i obecne zaokrąglenie importu2 miejsc; zmiana metody nie podmienia ceny ani klucza prowenancji. Nazwa ani pozycja w tabeli nie stanowią identyfikatora. Klucze cech katalogowych + jeden `inne`; stare niejednoznaczne cechy bez kluczy nie są zgadywane podczas edycji.
4. PP przechowuje confirmedBasis zamiast flagi; gotowość porównuje go z deterministycznym odciskiem aktualnych zależności I komórek, bez samego markera. Oczekiwany odcisk w żądaniu pochodzi z momentu otwarcia formularza; zapis porównuje go pod blokadą i stempluje nowy po zmianach. S1 definiuje pairwiseBasis. newVersionOf zeruje WR, resetuje methodConfirmed i usuwa confirmedBasis. Jawne potwierdzenie metody pozostaje osobną flagą z kontrolowanym setterem, bo zachowujemy poprzedni wariant w nowej wersji jako propozycję, nie potwierdzenie. Zmiana zależności kasuje aktualność, zachowując wartości robocze i własne opisy; nie oznaczamy starych mnożników jako automatycznie poprawnych. Usunięta transakcja/cecha nie przekazuje ocen nowej.
5. Obowiązkowo zachować szablon i projekcję KCS z bazy jako ścieżkę podpisu starszych zatwierdzonych snapshotów bez metody. Nowo zatwierdzane dokumenty mają jawnie metodę; nowy renderer dostaje techniczny `templateVersion`, nie wybiera biznesowej metody. Nie budować repozytorium wersji szablonów. Jest to ograniczona zgodność z bazą9b2903c, nie obietnica odtworzenia dowolnej wcześniejszej wersji systemu.
